...
from .integration import assessment_aliases_for_request, terminology_for_request
...
class EducationFrameworkIntegrationTests(TestCase):
    def setUp(self):
        self.organization = OrganizationProfile.objects.create(name="Integration Test School")
    ...
    def test_request_helpers_return_uganda_aliases_when_enabled(self):
        ...
        self.assertEqual(terms["external_exam"], "UNEB or External Exam")
        self.assertEqual(aliases["BOT"], "Beginning of Term Test")
        self.assertEqual(aliases["EOT"], "End of Term Examination")
    ...
    def test_template_tag_can_be_adopted_without_changing_existing_models(self):
        ...
        template = Template(
            "{% load education_terms %}{% education_term 'learner' %} | "
            "{% education_alias 'MOT' %}"
        )

        rendered = template.render(Context({"request": self.request}))

        self.assertEqual(rendered, "Pupil | Mid-Term Test")

    def test_local_aliases_are_disabled_in_neutral_mode(self):
        self.profile.use_local_terminology = False
        self.profile.save(update_fields=["use_local_terminology", "updated_at"])

        aliases = assessment_aliases_for_request(self.request)
        template = Template(
            "{% load education_terms %}{% education_alias 'BOT' 'Diagnostic Assessment' %}"
        )
