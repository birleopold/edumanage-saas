...
def get_institution_education_profile() -> InstitutionEducationProfile | None:
    ...
    return InstitutionEducationProfile.objects.select_related(
        "organization",
        "primary_framework",
    ).filter(organization=organization, is_active=True).first()


def get_request_campus_stage(request, *, stage_code: str | None = None) -> CampusEducationStage | None:
...
def terminology_for_request(request=None, *, stage_code: str | None = None) -> dict[str, str]:
    """Return effective labels for gradual adoption by existing views and templates."""

    profile = get_institution_education_profile()
    if profile is None:
        return dict(NEUTRAL_TERMINOLOGY)
    campus_stage = get_request_campus_stage(request, stage_code=stage_code) if request is not None else None
    return resolve_effective_terminology(profile=profile, campus_stage=campus_stage)


def assessment_aliases_for_request(request=None) -> dict[str, str]:
    """Return local aliases such as BOT, MOT, EOT and AOI when local terminology is enabled."""

    profile = get_institution_education_profile()
    if profile is None or not profile.use_local_terminology or not profile.primary_framework_id:
        return {}
    settings = dict(profile.primary_framework.default_settings or {})
    aliases = dict(settings.get("assessment_aliases") or {})
    aliases.update(dict((profile.settings or {}).get("assessment_aliases") or {}))
...
def external_exam_aliases_for_request(request=None) -> list[str]:
    ...
    custom_values = list((profile.settings or {}).get("external_exam_aliases") or [])
    return [str(value) for value in [*values, *custom_values] if str(value).strip()]
