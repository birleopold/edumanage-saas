...
def ensure_system_templates() -> tuple[dict[str, EducationStage], dict[str, AcademicFramework]]:
    ...
    for code, stage in stages.items():
        ...
        higher_education = code in {EducationStage.TERTIARY, EducationStage.UNIVERSITY}
        FrameworkStage.objects.update_or_create(
...
def ensure_institution_profile(
    ...
    *,
    ...
    framework_code = "UG-NATIONAL" if normalized_country == "UG" else "INTERNATIONAL-CUSTOM"
...
def resolve_terminology(
    *,
    ...
    """Resolve labels from defaults to the most specific explicit override."""
    ...
    if campus_stage:
        if framework_stage:
            stage_labels.update(
                {
                    "education_stage": framework_stage.local_name or campus_stage.stage.name,
...
def map_existing_levels(
    ...
    *,
    ...
    for level in Level.objects.all().order_by("order", "name"):
        inferred_stage = EducationStage.objects.get(code=infer_stage_code(level.name))
        existing = LevelStageMapping.objects.filter(
            profile=profile,
            legacy_level_id=level.pk,
        ).first()
        existing_settings = _mapping_settings(existing)
        is_manual = existing_settings.get("source") == MAPPING_SOURCE_MANUAL
        ...
        if existing and is_manual:
            ...
            existing.save(
                update_fields=["legacy_level_name", "settings", "updated_at"]
            )
            summary["updated"] += 1
            summary["manual_preserved"] += 1
            continue
...
def mapped_stage_ids_for_campus(
    profile: InstitutionEducationProfile,
    campus,
    *,
    all_mapped_stage_ids: set[int] | None = None,
) -> set[int]:
    """Infer the stages actually used by a campus from its active class groups.

    A single-campus institution safely falls back to all mapped stages. In a
    multi-campus institution, a campus without class-group evidence is left for
    explicit administrator configuration rather than receiving every stage.
    """

    if all_mapped_stage_ids is None:
        all_mapped_stage_ids = set(
            profile.level_mappings.values_list("stage_id", flat=True)
        )
    ...
    if level_ids:
        return set(
            profile.level_mappings.filter(
                legacy_level_id__in=level_ids
            ).values_list("stage_id", flat=True)
        )
    active_campus_count = profile.organization.campuses.filter(is_active=True).count()
    if active_campus_count == 1:
        return set(all_mapped_stage_ids)
    return set()
...
def enable_mapped_stages(profile: InstitutionEducationProfile) -> int:
    ...
    for campus in profile.organization.campuses.filter(is_active=True):
        stage_ids = mapped_stage_ids_for_campus(
            profile,
            campus,
        ...
        for stage in EducationStage.objects.filter(pk__in=stage_ids, is_active=True):
            ...
            _, created = CampusEducationStage.objects.get_or_create(
                ...
                defaults={
                    ...
                    "local_name": (
                        framework_stage.local_name if framework_stage else stage.local_name
                    ),
