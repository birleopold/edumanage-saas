...
class ExamPaperPolicy(models.Model):
    NUMERIC = "NUMERIC"
    ...
    class Meta:
        ordering = ("paper__exam", "paper__offering__course__name")

    def __str__(self):
        return f"Policy — {self.paper}"

    def clean(self):
        super().clean()
        errors = {}
        if self.responsible_teacher_id:
            offering = self.paper.offering
            ...
            if (
                ...
                and teacher_campus_id != offering_campus_id
            ):
                errors["responsible_teacher"] = (
                    "The responsible teacher belongs to another campus."
                )
        if self.makeup_for_id:
            if self.makeup_for_id == self.paper_id:
                errors["makeup_for"] = "A paper cannot be its own makeup paper."
            elif self.makeup_for.offering_id != self.paper.offering_id:
                errors["makeup_for"] = (
                    "The original and makeup papers must use the same course offering."
                )
            else:
                original_policy = getattr(self.makeup_for, "policy", None)
                if original_policy and not original_policy.allow_makeup:
                    errors["makeup_for"] = (
                        "The original paper does not permit a makeup paper."
                    )
        if self.absence_policy in {self.DEFERRED, self.MAKEUP_REQUIRED} and not self.allow_makeup:
            errors["allow_makeup"] = (
                "Deferred or makeup-required absence policies must allow a makeup paper."
            )
        if errors:
            raise ValidationError(errors)


class ExamScorePolicy(models.Model):
    PRESENT = "PRESENT"
    ABSENT = "ABSENT"
    EXCUSED = "EXCUSED"
    DEFERRED = "DEFERRED"
    ...
    def paper_policy(self):
        return getattr(self.score_record.paper, "policy", None)

    @property
    def effective_score(self):
        if self.makeup_completed_by_id:
            return self.makeup_completed_by.score
        ...
        if (
            ...
            and policy
            and policy.absence_policy == ExamPaperPolicy.ZERO
        ):
            return Decimal("0")
        return None
    ...
    def clean(self):
        ...
        if self.attendance_status in {self.DEFERRED, self.MAKEUP_PENDING}:
            if policy and not policy.allow_makeup:
                errors["attendance_status"] = (
                    "This paper does not permit deferred or makeup results."
                )
        if self.makeup_completed_by_id:
            ...
            replacement_policy = getattr(replacement.paper, "policy", None)
            if not replacement_policy or replacement_policy.makeup_for_id != self.score_record.paper_id:
                errors["makeup_completed_by"] = (
                    "The replacement score must come from a configured makeup paper."
                )
        if policy and policy.grading_mode == ExamPaperPolicy.COMPETENCY:
            if (
                self.attendance_status == self.PRESENT
                and self.competency_rating == self.NOT_ASSESSED
            ):
