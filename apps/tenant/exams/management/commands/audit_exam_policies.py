...
from ...policy_services import exam_policy_readiness


class Command(BaseCommand):
    help = "Run a read-only Phase 2 examination-policy audit."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--fail-on-incomplete",
            action="store_true",
            help="Return a non-zero exit code when examination policies are incomplete.",
        )
    ...
    def handle(self, *args, **options):
        ...
        for tenant in tenants:
            ...
            with tenant_context(tenant):
                ...
                self.stdout.write(
                    f"Papers: {readiness['paper_count']}; scores: {readiness['score_count']}; "
                    f"missing paper policies: {readiness['missing_paper_policy_count']}; "
                    f"missing score policies: {readiness['missing_score_policy_count']}."
                )
                self.stdout.write(
                    f"Invalid paper policies: {readiness['invalid_paper_policy_count']}; "
                    ...
                    f"report visibility mismatches: {readiness['report_visibility_mismatch_count']}."
                )
                if readiness["ready"]:
                    self.stdout.write(
                        self.style.SUCCESS("Phase 2 examination policies are ready.")
                    )
                else:
                    incomplete += 1
                    self.stdout.write(
                        self.style.WARNING(
                            "Phase 2 examination policies need attention. Existing scores were not changed."
                        )
                    )
        self.stdout.write(
        ...
        if options.get("fail_on_incomplete") and incomplete:
            raise CommandError(
                f"Examination-policy audit found {incomplete} incomplete tenant(s)."
            )
