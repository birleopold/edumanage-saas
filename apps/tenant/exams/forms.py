...
class QuestionBankForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if question_type in [QuestionBank.MCQ, QuestionBank.TRUE_FALSE]:
            ...
            if question_type == QuestionBank.MCQ:
                if correct_option.upper() not in ["A", "B", "C", "D"]:
                    self.add_error("correct_option", "For MCQ, correct option must be A, B, C, or D.")
            elif question_type == QuestionBank.TRUE_FALSE:
                if correct_option.upper() not in ["A", "B"]:
                    self.add_error("correct_option", "For True/False, correct option must be A or B.")
        
        if question_type in [QuestionBank.SHORT_ANSWER, QuestionBank.FILL_BLANK]:
            if not cleaned.get("correct_answer"):
                raise forms.ValidationError(f"Correct answer is required for {question_type} questions.")
...
class ExamQuestionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if paper and question:
            if question.course_id != paper.offering.course_id:
