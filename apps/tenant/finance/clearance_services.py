...
def current_academic_term():
    return AcademicTerm.objects.filter(is_current=True).select_related("year").order_by("-year__name", "order").first()
...
def resolve_clearance_policy(student, access_type: str, academic_term=None):
    ...
    qs = ClearancePolicy.objects.filter(is_active=True, access_type=access_type).select_related(
    ...
    candidates.sort(key=lambda policy: (policy.priority, _specificity(policy), policy.pk), reverse=True)
    return candidates[0] if candidates else None


def finance_summary_for_policy(student, policy: ClearancePolicy | None, academic_term=None) -> FinanceSummary:
    term = academic_term or (policy.academic_term if policy and policy.academic_term_id else None) or current_academic_term()
    ...
    if invoiced > 0:
        percentage = (paid * Decimal("100") / invoiced).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    else:
...
def resolve_clearance_override(student, access_type: str, policy=None, academic_term=None):
    ...
    qs = qs.filter(Q(access_type=ClearanceOverride.ALL) | Q(access_type=access_type))
    if academic_term:
        qs = qs.filter(Q(academic_term__isnull=True) | Q(academic_term=academic_term))
    else:
        qs = qs.filter(academic_term__isnull=True)
    if policy:
    ...
    return qs.select_related("policy", "academic_term", "approved_by").order_by("-valid_until", "-created_at").first()
...
def _failure_reason(policy: ClearancePolicy, summary: FinanceSummary) -> str:
    ...
    if policy.rule_type == ClearancePolicy.FULL_PAYMENT:
        return f"Outstanding balance is {summary.outstanding_balance}; full payment is required."
    if policy.rule_type == ClearancePolicy.MINIMUM_PERCENTAGE:
        return (
...
def evaluate_clearance(student, access_type: str, academic_term=None) -> ClearanceDecision:
    term = academic_term or current_academic_term()
    policy = resolve_clearance_policy(student, access_type, academic_term=term)
    summary = finance_summary_for_policy(student, policy, academic_term=term)
    if not policy:
    ...
    if override:
        return ClearanceDecision(
            ...
            reason=f"Access granted by approved override: {override.reason}",
        )
    ...
    return ClearanceDecision(
        ...
        allowed=advisory,
        advisory=advisory,
        decision_code=ClearanceDecisionLog.ADVISED if advisory else ClearanceDecisionLog.BLOCKED,
        reason=reason,
    )


def record_clearance_decision(decision: ClearanceDecision, *, source=ClearanceDecisionLog.ADMIN, checked_by=None):
    return ClearanceDecisionLog.objects.create(
    ...
    for access_type, name in templates:
        ...
        if apply and not exists:
            ClearancePolicy.objects.create(
                ...
                user_message="Please contact the finance office to confirm your fee clearance.",
                is_active=False,
            )
    return rows
...
def clearance_readiness():
    ...
    access_types = {policy.access_type for policy in policies if policy.is_active and policy not in invalid}
    missing_access_types = [
        code for code, _label in ClearancePolicy.ACCESS_TYPE_CHOICES if code not in access_types
    ]
    return {
