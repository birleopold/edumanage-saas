...
class Payment(models.Model):
    ...
    MOBILE_NETWORK_CHOICES = (
        ...
        (MOBILE_OTHER, "Other mobile wallet"),
    )
    ...
    mobile_network = models.CharField(
        max_length=16,
        choices=MOBILE_NETWORK_CHOICES,
        blank=True,
        help_text="When paying by mobile money, which network was used (for reconciliation).",
    )
...
class OutboundMessageLog(models.Model):
    ...
    class Meta:
        ordering = ("-created_at",)
        indexes = [
            models.Index(fields=["message_type", "channel", "status"]),
...
class WebhookRetryQueueItem(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["is_active", "next_attempt_at"]),
...
class CommunicationTemplate(models.Model):
    """
    Editable SMS/WhatsApp copy used as a library for fee reminders, alerts, and broadcasts.
    Bodies may include placeholders such as {{school_name}}, {{parent_name}}, {{student_name}},
    {{amount}}, {{currency}}, {{due_date}}, {{invoice_ref}}, {{announcement_title}}.
    """

    ANY = "ANY"
    SMS = "SMS"
    WHATSAPP = "WHATSAPP"
    CHANNEL_HINT_CHOICES = (
        (ANY, "Any channel"),
        (SMS, "SMS"),
        (WHATSAPP, "WhatsApp"),
    )
