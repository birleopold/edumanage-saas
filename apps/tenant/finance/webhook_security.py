...
def install_webhook_delivery_guard():
    """Revalidate every target immediately before initial and retry delivery."""
    from . import services

    original = services._deliver_webhook
    if getattr(original, "_edumanage_target_guard", False):
        return

    @wraps(original)
    def guarded(endpoint, event_type, payload):
        try:
            validate_webhook_target(endpoint.target_url)
            ...
            return {
                ...
                "success": False,
                "response_body": "",
                "error_message": f"blocked_webhook_target:{message}",
            }
        return original(endpoint, event_type, payload)

    guarded._edumanage_target_guard = True
    services._deliver_webhook = guarded
