...
def get_class_rank(student_id: int, term_id: int, stream_id: Optional[int] = None) -> dict:
    ...
    for student in students:
        average = calculate_term_average(student.id, term_id)
        if average is not None:
            student_averages.append({"student_id": student.id, "average": average})

    student_averages.sort(key=lambda item: item["average"], reverse=True)

    rank = None
    for index, item in enumerate(student_averages, start=1):
...
def get_subject_statistics(offering_id: int) -> dict:
    ...
    passing_scores = [score for score in all_scores if score >= Decimal("50")]
