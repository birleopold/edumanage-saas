...
def admin_poll_list(request):
    polls = Poll.objects.select_related("campus", "created_by").prefetch_related("options").annotate(vote_total=Count("votes")).order_by("-created_at")
...
def admin_poll_edit(request, pk):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        return redirect("admin_poll_detail", pk=poll.pk)
    return render(request, "portals/admin/polls/form.html", {"form": form, "poll": poll, "mode": "edit"})
...
def admin_poll_option_add(request, pk):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        return redirect("admin_poll_detail", pk=poll.pk)
    return render(request, "portals/admin/polls/option_form.html", {"form": form, "poll": poll})


@login_required
def poll_list(request):
    polls = polls_for_user(request)
    return render(request, "portals/polls/list.html", {"polls": polls})


@login_required
def poll_detail(request, pk):
    poll = get_object_or_404(polls_for_user(request, include_closed=True).prefetch_related("options"), pk=pk)
