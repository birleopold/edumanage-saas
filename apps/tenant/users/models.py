...
class Role(models.Model):
    ...
    STUDENT = "STUDENT"
    PARENT = "PARENT"
    CODE_CHOICES = ((ADMIN, "Admin"), (CAMPUS_ADMIN, "Campus Admin"), (PRINCIPAL, "Principal"), (TEACHER, "Teacher"), (STUDENT, "Student"), (PARENT, "Parent"))
...
class UserRole(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        super().save(*args, **kwargs)
    def __str__(self) -> str:
        return f"{self.user} -> {self.role} ({self.campus})" if self.campus else f"{self.user} -> {self.role}"
...
class PasswordSetupToken(models.Model):
    ...
    def create_for_user(cls, user: User, created_by=None, validity_hours: int = 72):
        ...
        return cls.objects.create(user=user, token=token, expires_at=timezone.now() + timedelta(hours=validity_hours), created_by=created_by)
