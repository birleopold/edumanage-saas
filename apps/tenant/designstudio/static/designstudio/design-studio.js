(() => {
  ...
  function remember(){ if(!editable)return; undoStack.push(deep(state)); if(undoStack.length>60) undoStack.shift(); redoStack.length=0; updateHistoryButtons(); }
  function updateHistoryButtons(){ byId('undo-btn').disabled=!editable||!undoStack.length; byId('redo-btn').disabled=!editable||!redoStack.length; }
  function undo(){ if(!editable||!undoStack.length)return; redoStack.push(deep(state)); state=undoStack.pop(); selectedId=null; render(); updateHistoryButtons(); }
  function redo(){ if(!editable||!redoStack.length)return; undoStack.push(deep(state)); state=redoStack.pop(); selectedId=null; render(); updateHistoryButtons(); }
  ...
  function addElement(type,binding=''){
    if(!editable)return; remember(); const w=parseFloat(widthInput.value),h=parseFloat(heightInput.value);
  ...
  function startPointer(event,el,resizing){event.preventDefault();event.stopPropagation();selectElement(el.id);if(!editable)return;remember();const startX=event.clientX,startY=event.clientY,ox=Number(el.x),oy=Number(el.y),ow=Number(el.width),oh=Number(el.height),scale=mmToPx*zoom;const move=e=>{const dx=(e.clientX-startX)/scale,dy=(e.clientY-startY)/scale;if(resizing){el.width=Math.max(1,ow+dx);el.height=Math.max(1,oh+dy);}else{el.x=ox+dx;el.y=oy+dy;}clampElement(el);renderCanvas();syncProperties();};const up=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up);}
  ...
  function bindProperty(id,key,parser=v=>v){const input=byId(id);input.addEventListener('change',()=>{if(!editable)return;const el=selected();if(!el)return;remember();el[key]=parser(input.type==='checkbox'?input.checked:input.value);clampElement(el);render();});input.addEventListener('input',()=>{if(!editable)return;const el=selected();if(!el)return;el[key]=parser(input.type==='checkbox'?input.checked:input.value);clampElement(el);renderCanvas();});}
  ...
  function deleteSelected(){if(!editable||!selectedId)return;remember();currentPage().elements=currentPage().elements.filter(e=>e.id!==selectedId);selectedId=null;render();}
  document.querySelectorAll('[data-add-element]').forEach(b=>b.onclick=()=>addElement(b.dataset.addElement));
  document.querySelectorAll('[data-add-binding]').forEach(b=>b.onclick=()=>{const kind=b.dataset.kind;addElement(kind==='results_table'?'results_table':kind,b.dataset.addBinding);});
  byId('delete-element-btn').onclick=deleteSelected;canvas.onclick=()=>{selectedId=null;renderCanvas();renderProperties();};
  byId('add-page-btn').onclick=()=>{if(!editable)return;remember();state.pages.push({id:uid('page'),name:`Page ${state.pages.length+1}`,elements:[]});activePage=state.pages.length-1;selectedId=null;render();};
  ...
  [widthInput,heightInput].forEach(input=>input.onchange=()=>{if(!editable)return;remember();currentPage().elements.forEach(clampElement);render();});
  const bgInput=byId('background-input');bgInput.onchange=()=>{if(!editable||!bgInput.files[0])return;const reader=new FileReader();reader.onload=e=>{backgroundPreview=e.target.result;renderCanvas();};reader.readAsDataURL(bgInput.files[0]);};byId('background-fit').onchange=()=>{if(editable)renderCanvas();};
  function save(){if(!editable)return;byId('design-json').value=JSON.stringify(state);byId('save-width').value=widthInput.value;byId('save-height').value=heightInput.value;byId('save-background-fit').value=byId('background-fit').value;const target=byId('save-background');if(bgInput.files.length){const dt=new DataTransfer();dt.items.add(bgInput.files[0]);target.files=dt.files;}byId('save-form').requestSubmit();}
  const saveBtn=byId('save-btn');if(saveBtn)saveBtn.onclick=save;
