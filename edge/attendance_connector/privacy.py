...
REDACTED = "[REDACTED_BY_EDUMANAGE_EDGE]"
...
def scrub_edge_payload(value, *, depth=0):
    """Keep attendance metadata but remove biometric enrollment/image material locally."""
    if depth >= MAX_DEPTH:
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            text_key = str(key)
            output[text_key] = REDACTED if _is_sensitive_key(text_key) else scrub_edge_payload(item, depth=depth + 1)
        return output
    if isinstance(value, (list, tuple)):
        cleaned = [scrub_edge_payload(item, depth=depth + 1) for item in value[:MAX_LIST_ITEMS]]
